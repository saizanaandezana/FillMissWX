FillMissWX=function(declat, declon,StnRadius,date_min,date_max,method,alfa=2){
  if (method == "IDW"){
    ...
    if("tmax" %in% colnames(modeldata)){
      modeldata$tmax=modeldata$tmax*(1/(WXDistance^alfa))
    } else {
      modeldata$tmax=NA
    }
    
    if("tmin" %in% colnames(modeldata)){
      modeldata$tmin=modeldata$tmin*(1/(WXDistance^alfa))
    } else{
      modeldata$tmin=NA
    }
    modeldata$DenPrcp=0
    ...
    modeldata$DenTmax=0
    if("prcp" %in% colnames(modeldata)){
      modeldata$DenPrcp[!is.na(modeldata$prcp)]=(1/(WXDistance^alfa))
    }
    if("tmax" %in% colnames(modeldata)){
      modeldata$DenTmax[!is.na(modeldata$tmax)]=(1/(WXDistance^alfa))
    }
    if("tmax" %in% colnames(modeldata)){
      modeldata$DenTmin[!is.na(modeldata$tmin)]=(1/(WXDistance^alfa))
    }
    for (i in 2:length(ustns[,1])){
      WXStn=ustns[order(ustns$distance),]$id[i]
      WXDistance=ustns[order(ustns$distance),]$distance[i]
