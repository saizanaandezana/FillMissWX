    ...
    for (i in 2:length(ustns[,1])){
      ...
      if (length(WXData$id)>5){
        ...
        if("prcp.y" %in% colnames(modeldata)){
          ...
          modeldata$prcpElev[is.na(modeldata$prcp.x)& !is.na(modeldata$prcp.y)]=WXElev
        ...
        if("tmin.y" %in% colnames(modeldata)){
          ...
          modeldata$tminElev[is.na(modeldata$tmin.x) & !is.na(modeldata$tmin.y)]=WXElev
        ...
        if("tmax.y" %in% colnames(modeldata)){
          ...
          modeldata$tmaxElev[is.na(modeldata$tmax.x)&!is.na(modeldata$tmax.y)]=WXElev
